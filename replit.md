...
- **Portal authentication architecture fix (July 18, 2025)**: Fixed critical architectural issue where portal users were incorrectly created as system users with 'patient' role. Completely redesigned portal authentication to properly separate external users (leads/contacts) from system users (SDR, Health Coach, Admin). Added passwordHash, portalAccess, and lastPortalLogin fields to leads and contacts tables. Updated portal signup to create leads directly with portal access instead of system users. Modified portal authentication (loginExternalUser, validateSession) to authenticate against leads/contacts tables rather than users table. Fixed activity logging to handle external users without requiring system user IDs. Removed 'patient' from userRoleEnum as it's not a valid system role. Updated automation triggers to work with the new lead-based portal signup flow. This creates proper separation between internal CRM users and external portal users, maintaining data integrity and security boundaries.
- **Lead-focused portal dashboard with HHQ integration (July 18, 2025)**: Created professional lead-focused portal dashboard that prominently displays Health History Questionnaire (HHQ) as the primary next step for new leads. Implemented HHQ completion status tracking with proper database table creation (health_questionnaires). Fixed portal authentication endpoints (/api/portal/login, /api/portal/logout) and token handling. Added HHQ status endpoint (/api/portal/hhq/status) with proper authentication. Created dedicated HHQ portal page (/portal/hhq) with multi-step form integration. Dashboard shows different content based on user type: leads see HHQ as primary focus with progress tracking, contacts see standard patient dashboard. HHQ form properly connects to lead records and tracks completion through all steps (health questions, signature, payment, appointment booking). Once completed, HHQ disappears from dashboard as requested. Fixed all authentication issues and API endpoint connections for seamless portal experience.
- **Portal API routing and infinite re-render fixes (July 18, 2025)**: Fixed critical React "Too many re-renders" error by eliminating unstable `useMemo` with `localStorage.getItem('portalToken')` and empty dependencies. Resolved API routing conflicts where Vite middleware was intercepting portal API requests and returning HTML instead of JSON. Added direct `/login` endpoint to portal routes for proper authentication. Fixed portal token management by moving token retrieval inside callbacks rather than memoizing with empty dependencies. All portal endpoints now return proper JSON responses and authentication flow works correctly without infinite re-renders.
- **Complete lead-to-contact conversion workflow (July 18, 2025)**: Successfully implemented full lead-to-contact conversion system with fixed portal authentication prioritization. When leads complete HHQ workflow and reach "Booking: Paid/Booked" status, they automatically convert to contacts with "Intake" stage. Updated portal authentication to check contacts first, then leads, ensuring converted users access their contact record. All appointments transfer from lead to contact during conversion, maintaining continuity with assigned health coach (Sarah Wilson). Portal authentication now properly prioritizes contact records over lead records for users who have completed the conversion process. Complete end-to-end workflow: portal signup → lead creation → HHQ completion → automatic contact conversion → seamless portal access to appointments.
- **HHQ form infinite re-render fix (July 18, 2025)**: Fixed additional infinite re-render issue in HHQ form component where `setState` calls were being made directly during render when `existingHhq` data was present. Moved all state updates (`setCurrentStep`, `setHhqData`) inside `useEffect` with proper dependencies to prevent render-time state changes. HHQ form now properly handles existing questionnaire data without causing React render limit errors. All HHQ workflow steps (questionnaire, signature, payment, appointment booking) now work correctly without infinite re-renders.
- **Database enum alignment and HHQ workflow completion (July 18, 2025)**: Fixed critical database enum mismatch where code was using "Booking: Needs Scheduling" and "Booking: Paid/ booked" but database had "Booking: Paid/Not Booked" and "Booking: Paid/Booked". Updated all enum references across schema.ts, storage.ts, businessDefinitions.ts, and NodeProperties.tsx to match actual database values. Verified complete HHQ workflow now functions correctly: health questions save properly, signature recording works, payment processing succeeds, appointment booking completes, and lead status updates correctly through all stages. Added HHQ reset functionality for testing purposes.
- **Signature capture and persistence implementation (July 18, 2025)**: Added proper signature data capture and storage to HHQ workflow. Added signatureData text field to health_questionnaires table and updated storage methods to accept and persist signature information. Enhanced SignatureStep component with full name input field for electronic signature capture. Fixed signature state management in HHQ form to properly handle existing signatures when users return to the form. Signature data now persists correctly and is displayed in step completion status. Created comprehensive signature validation requiring both agreement checkbox and name input before signing. System now maintains signature integrity across page reloads and properly tracks signature completion status.
- **Portal appointments integration fix (July 18, 2025)**: Fixed critical issue where appointments weren't showing in portal appointments tab despite successful HHQ workflow completion. Issue was placeholder portal appointments endpoint returning empty array instead of actual appointments. Implemented proper appointment fetching logic supporting both leads and contacts with correct database queries. Added missing appointments import to portal routes. Fixed database schema alignment for appointments table (scheduledAt/duration vs startTime/endTime). Portal now correctly displays all appointments for authenticated users including HHQ consultation appointments. Complete end-to-end workflow now functional: portal signup → HHQ completion → appointment booking → portal appointment display.